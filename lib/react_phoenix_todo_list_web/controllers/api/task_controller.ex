defmodule ReactPhoenixTodoListWeb.Api.TaskController do
  use ReactPhoenixTodoListWeb, :controller

  alias ReactPhoenixTodoList.Todo
  alias ReactPhoenixTodoList.Todo.Task

  action_fallback ReactPhoenixTodoListWeb.FallbackController
  ...
  def create(conn, %{"task" => task_params}) do
    with {:ok, %Task{} = task} <- Todo.create_task(task_params) do
      conn
      |> put_status(:created)
      |> put_resp_header("location", ~p"/api/api/tasks/#{task}")
